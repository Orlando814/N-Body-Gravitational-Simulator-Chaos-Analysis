...
def find_lagrange_123(gradient_func, b1_pos: np.ndarray, b2_pos: np.ndarray, m1: float, m2: float, omega: float,
    ...
    def f(axis):
            return gradient_func(np.array([axis, 0, 0]), pos, mass, omega, g)[0]
...
def find_lagrange_45(b1_pos: np.ndarray, b2_pos: np.ndarray, m1: float, m2: float) -> np.ndarray:
...
lagrange_45 = find_lagrange_45(r1, r2, m1, m2)
